...
const handler = async (req, res) => {
  switch (req.method) {
    case "GET":
      await getTiposSituacaoVaga(req, res);
      break;
    case "POST":
      await addTipoSituacaoVaga(req, res);
      break;
...
const getTiposSituacaoVaga = async (req, res) => {
...
const addTipoSituacaoVaga = async (req, res) => {
  const { entity } = req.query;
  const { nome } = req.body;
  try {
    const table = `${entity}_Tipos_Situacoes_Vaga`;
    // const query = await prisma.ba_Tipos_Situacoes_Vaga.create({
